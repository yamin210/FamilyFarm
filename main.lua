...
function alleMaschinenHack()
    gg.toast("Starte Maschinen Hack...")
    ...
    if #res1 > 0 then
        ...
        for _, v in ipairs(targets1) do
            gg.searchNumber(tostring(v), gg.TYPE_DWORD)
            local found = gg.getResults(9999)
            for _, item in ipairs(found) do item.value = "0" end
            gg.setValues(found)
            gg.searchNumber("0~900", gg.TYPE_DWORD) -- Refresh
        end
    ...
    gg.alert("✅ Alle Maschinen auf 0 gesetzt!")
end
